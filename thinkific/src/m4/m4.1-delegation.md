...
The list is saved in the agent's state, so it persists from one turn to the next. On a long task the agent always has an explicit, up-to-date plan to check against instead of trying to hold the whole thing in its head. Think of it as writing the steps on a whiteboard and ticking them off as it goes.
...
- A subagent is **a full agent in its own right**, with its own prompt and isolated context, and optionally its own tools and model, invoked through the built-in **`task`** tool.
