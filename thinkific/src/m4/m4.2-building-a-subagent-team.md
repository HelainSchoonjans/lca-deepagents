...
Everything the agent produced runs on a sandboxed `StateBackend`, so the script mirrors it out of agent state and onto disk under `python/m4/output/`. The finished newsletter sits alongside each researcher's raw `/research/<genre>/` archive:
...
<MCQ
    ...
    correctIndex={1}
    explanation="A subagent returns one message to the editor. Returning only the polished segment (not the bulky raw results) keeps the editor's context clean. The private file is simply where the researcher keeps the material it needs while drafting."
/>
