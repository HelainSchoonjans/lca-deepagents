...
The default sandbox includes Python and common packages. The sandbox starts fresh each run; no state persists between invocations unless you upload files first.
...
Build an agent that writes a Python script to a sandbox, executes it, and returns the output. You will see the write → execute sequence in LangSmith as a chain of tool calls.
...
```python {11,12}
# python/m2/m2_3_sandbox_agent.py
import sys
...
```bash
cd python
uv run ./m2/m2_3_sandbox_agent.py
```
...
<pre style="max-height:260px;overflow-y:auto;background:#0d1117;color:#e6edf3;padding:16px;border-radius:6px;font-size:12px;font-family:'IBM Plex Mono',monospace;line-height:1.6;white-space:pre-wrap;">Sandbox: lca-deepagents-lab  (id: 41ee1fb1-757a-48f9-b288-2ea69bedc4d2)
Done! The script has been saved to `/fib.py` and executed. It prints the first 15 Fibonacci numbers:
...
The sandbox id in the first line links to your LangSmith trace. Look for the `write_file` → `execute` tool call sequence in the trace.
