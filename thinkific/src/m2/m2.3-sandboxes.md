[For translation, open lesson in new tab and use Chrome translate](https://langchain-ai.github.io/lca-deepagents/m2/m2.3-sandboxes.html)
...
**Script (will be recorded):** This lesson builds on the filesystem backends from last time. Those backends gave the agent file tools: `read_file`, `write_file`, and so on. `LocalShellBackend` and sandbox backends add one more: `execute`, which lets the agent actually run shell commands.

[Show the diagram]

The diagram is simple. Both `LocalShellBackend` and `Sandbox` backends sit under the same Backend node, and both unlock the `execute` tool in addition to the standard filesystem surface. The agent sees the same tools either way; what changes is where those commands actually run.

[Scroll to "The execute tool"]

When the agent calls `execute`, it gets back stdout, stderr, and an exit code as a tool result. That means if something fails, the agent can see the error and fix it - and this is one of the powerful things a sandbox enables.

[Scroll to "How do you choose?"]

The choice comes down to isolation. `LocalShellBackend` runs commands directly on your machine, no sandboxing, no process isolation. Even if you set a `root_dir`, that only scopes the filesystem tools. The `execute` tool still runs with full host permissions, so the agent could theoretically run anything. That makes it fine for local development and personal tooling, but not for production or anything involving untrusted input.

Sandboxes are the right choice when the agent needs to run arbitrary code safely - coding agents, data analysis, anything where you can't fully predict what the agent will execute.

[Show the sandbox models diagram]

There are two models for using a sandbox. On the left, Agent in Sandbox - the agent itself runs inside the sandbox. API keys have to live inside it and updates require rebuilding the image. On the right, Sandbox as a Tool - the agent runs on your host and calls the sandbox via `execute` and filesystem tools. API keys stay on your machine, the agent code is easy to update, and sandbox failures don't affect agent state. This is the model we use in this course.

[Scroll to "Configuring a LangSmith sandbox"]

Setting one up is straightforward. You create a sandbox client, provision a sandbox with `create_sandbox`, wrap it in `LangSmithSandbox`, and pass it as the backend. The try/finally block is important - sandboxes are billed resources and you always want to clean them up, even if the agent raises an error. The sandbox starts completely fresh each run, so anything you need inside it has to be explicitly uploaded beforehand.

[Scroll to LocalShellBackend table]

`LocalShellBackend` has a few configuration options worth knowing: `root_dir` scopes the filesystem tools, `timeout` limits how long each `execute` call can run, and `inherit_env` controls whether the agent inherits your current environment variables. Just remember - `root_dir` does not restrict what `execute` can do. There's a security warning on the page that's worth reading if you plan to use this backend.

Head over to the labs to see both patterns in action: Lab 1 is a basic sandbox agent that writes and runs a script, and Lab 2 uploads a database file, queries it, and retrieves a generated chart back from the sandbox.
...
<img width="720" alt="Auth Proxy Pattern: sandbox makes requests without credentials, proxy sidecar injects Authorization header on outbound requests" src="images/m2.3-auth-proxy.png" />
</div>
...
`LocalShellBackend` gives the agent access to the local filesystem and shell. Even with `root_dir` set, the `execute` tool runs with full host permissions; file scoping does not limit what shell commands can do. Unless your intention is to build a desktop agent designed to work and operate on local files and commands (a coding agent for example), a sandbox is a better choice.
...
The next lesson covers the interpreter and how the agent writes and executes code directly in the main loop.
...
Create `python/m2/m2.3_sandbox_agent.py`:
...
<MCQ
    ...
    correctIndex={1}
    explanation="root_dir scopes the filesystem tools only. The execute tool still runs with full host permissions regardless of root_dir, which is why LocalShellBackend is unsuitable for production."
/>
...
<MCQ
    ...
    choices='["LocalShellBackend encrypts all file operations; sandbox backends do not", "Sandbox backends require an API key; LocalShellBackend does not", "With LocalShellBackend, the execute tool runs with full host permissions and is not isolated from the rest of the system", "LocalShellBackend only allows read operations; sandbox backends allow both read and write"]'
    correctIndex={2}
    explanation="LocalShellBackend gives the execute tool full host permissions. root_dir limits filesystem tools but not execute, so the agent can run any command on your machine. A sandbox backend isolates execution entirely, making it safe for production use with untrusted input."
/>
