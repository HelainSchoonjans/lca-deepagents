...
Both invokes run as a single LangGraph trace with two model calls. Open the trace in LangSmith.

**Memory is always present:**

Click the first `ChatAnthropic` call → **Input** tab → expand the System message → scroll to the bottom. You will see an `[agent_memory]` block containing the raw content of `AGENTS.md`. This is what `MemoryMiddleware` injects on every LLM call, regardless of what the user asked.

**The memory write:**

Scroll down to the second model call (the "remember ruff" turn). In the **Output** tab, the agent first calls `read_file` on `memories/AGENTS.md` to read the current content, then `edit_file` to write the updated version. The `edit_file` call is the moment the new information is persisted.
