...
**Script (will be recorded):** Memory files are markdown files loaded at agent startup and injected into the system prompt on every LLM call. Unlike skills, which load on demand when a task matches, memory is always present.
...
<MCQ
    question="What does Deep Agents create when file paths are passed to the memory parameter of create_deep_agent?"
    choices='["A MemoryMiddleware that reads the file and injects its content into the system prompt on every call", "A FilesystemBackend rooted at the memory directory", "A checkpointer that snapshots memory file content between runs", "A SQLite database for storing and indexing the memory contents"]'
    correctIndex={0}
