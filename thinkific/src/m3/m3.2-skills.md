...
**Script (will be recorded):** A skill is a directory containing a SKILL.md file. Skills follow an open standard, which means they're portable across agents and shareable across teams.

[Show diagram]

The directory structure is simple. Under your skills/ folder, each skill gets its own named subdirectory. Inside that, SKILL.md is the only required file, highlighted here in yellow. Everything else is optional: a scripts/ folder for executable code, references/ for documentation, assets/ for templates or resources. Importantly, the agent can't see any of those optional files unless the skill body explicitly mentions them.

[Slide 2]

This slide shows two skills side by side. Notice what's shown underneath each SKILL.md: just the name and description. Those are the only two fields injected into the system prompt on every call. The full body - the steps, output format, everything else - is only loaded when the agent activates the skill by calling read_file.

[Next diagram]

And this is what that looks like inside the file. The top block is the YAML frontmatter: always in context, always compact. Below it are the markdown instructions: the actual behaviour, like the BANT framework steps shown here as an example, and this stays out of context until activation.

So the frontmatter is always present but compact, and the instructions are only loaded when needed. That's the core of how skills stay lightweight.

[Scroll to the frontmatter fields table]

There are only two required frontmatter fields: name and description. The name must be lowercase with hyphens and must match the directory name. The description is a single sentence describing when to use the skill; we'll come back to why that matters. The rest of the fields [highlight allowed-tools, compatibility, and metadata] are optional.
...
**Script (will be recorded):** Skills use a three-stage pattern called progressive disclosure, which is designed to keep the agent's context lean. The idea is: don't load what you don't need yet.

[Scroll to and highlight the Skills System code block]

Stage one is discovery. At startup, the SDK reads the name and description from each skill's frontmatter and automatically injects them into the system prompt. The SDK generates this section, not you. This is what it looks like. Both skills are listed [highlight] with their descriptions and the path to their SKILL.md. That's all that's in context at this point: just the names and descriptions, nothing from the skill bodies.

[Scroll to "Activation"]

Stage two is activation. When a user's request matches a skill's description, the agent calls read_file on the path shown in the system prompt, loading the full SKILL.md into context for that turn. This is the moment the instructions actually arrive.

[Scroll to "Execution"]

Then stage three is execution; the agent follows the instructions in the skill body, calling whatever tools it needs.

Head over to the lab to see how the skills are wired up and watch both activation stages play out in LangSmith!
...
**Script (will be recorded):** [Open m3.2_scratch_agent_skills.py in VS Code]

This is the whole agent! The point of this lab is to see skills in action so let's get right to it.

[Highlight create_deep_agent]

There are only three things being passed here: the model, a system prompt saying "You are a sales assistant", and the skills= parameter pointing to the skills/ directory. That's it. The SDK scans that directory, reads the frontmatter from each SKILL.md, and generates the Skills System block in the system prompt automatically.

[Highlight the agent.invoke line]

The agent is invoked with a single message: qualifying a lead at Acme Corp. Run this and you'll see the agent activate the qualify-lead skill; it calls read_file on qualify-lead/SKILL.md, loads the BANT framework instructions, and follows them.

[Run it in terminal]

Let's run it. Watch the output: the agent works through the qualification steps and classifies the lead.

Now try swapping the message for: "Write a cold outreach message for a prospect at a mid-size logistics company." That will activate draft-pitch instead.

[Open LangSmith]

Let's open both runs in LangSmith and compare the system messages. In both, you'll see the Skills System section listing both skill names and descriptions. But the read_file call will target a different SKILL.md in each run. That's the activation stage made visible. The description was the only thing that determined which skill got loaded.
