...
Skills are files, so they must be available through the agent's backend. The `skills` argument points to a directory path in that backend, not to an arbitrary local path the model can magically see.
